...
public class BitTorrent implements EDProtocol {
	...
	/**
	 *	The Node type used to tell its behaviour
	 */
	int nodeType;
	...
	*/
	...
	public double getDistance(Node n, Node s)
	{
		double distance=0;
		distance=Math.sqrt(Math.pow((s.getCoordX()-n.getCoordX()),2)+Math.pow((s.getCoordY()-n.getCoordY()),2));
		return distance;
	}
	/**
	...
	public int getThisNodeCoordX(){
		return this.thisNodeCoordX;
	}
...
/**	}catch(Exception e){
	...
	/**
	 *	Sets the type of the local node
	 *	@param value The value of its type : each type has a different behavior
	 */
